...
class AnimationButtons():
    ...
    def slider(frame_names):
        steps = [dict(
            args=[[i], dict(frame={'duration': 300, 'redraw': False}, mode="immediate", transition={'duration': 300})],
...
def create_data_bagging_utils(d=4, number_of_members=1, n_samples=1000):
    ...
    tags = np.hstack(
        [np.repeat(samples_["tag"], samples_["samples_of_half"].shape[0]) for samples_ in built_list.values()])
    return samples, tags