...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        init_weights = np.random.normal(loc=0, scale=1.0, size=X.shape[1]) / np.sqrt(X.shape[1])

        module = None
        if self.penalty_ == "none":
            module = LogisticModule(init_weights)
        elif self.penalty_ == "l1":
            module = RegularizedModule(LogisticModule(), L1(), self.lam_, include_intercept=self.include_intercept_, weights=init_weights)
        elif self.penalty_ == "l2":
            module = RegularizedModule(LogisticModule(), L2(), self.lam_, include_intercept=self.include_intercept_, weights=init_weights)
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        return np.mean(self._predict(X) != y)
