...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.power(norm(self.weights, ord=2), 2)
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return np.sum(np.log(1 + np.exp(X_weights)) - y * X_weights) / X.shape[0]
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return (sigmoid @ X - y @ X) / X.shape[0]
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        f_w = self.fidelity_module_.compute_jacobian(**kwargs)
        if self.include_intercept_:
            r_w = np.concatenate(([0], self.regularization_module_.compute_jacobian(**kwargs)))
        else:
            r_w = self.regularization_module_.compute_jacobian(**kwargs)
        return f_w + self.lam_ * r_w
