...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        n_samples = X.shape[0]
        self.D_ = np.full((n_samples,), 1 / n_samples)  # Initialize sample weights uniformly

        for t in range(self.iterations_):


            # Instantiate a new weak learner
            learner = self.wl_()

            # Fit the weak learner using current weights
            ...
            weighted_error = np.sum(self.D_ * (y_pred != y))
            ...
            # Update weights D
            self.D_ *= np.exp(-alpha_t * y * y_pred)
            self.D_ /= np.sum(self.D_)  # Normalize weights to sum up to 1
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for i in range(min(self.iterations_, T)):
            # Get the current model and weight
            model = self.models_[i]
            weight = self.weights_[i]

            # Predict using the current model and update predictions
            predictions += weight * model.predict(X)

        # Return predictions with sign function applied to convert to class labels
